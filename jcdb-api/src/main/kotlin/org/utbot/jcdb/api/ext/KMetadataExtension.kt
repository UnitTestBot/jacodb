...
fun JcType.updateNullability(kmType: KmType?, nullableByAnnotations: Boolean): JcType {
    if (this !is JcRefType)
        return this

    if (kmType != null)
        return relaxNullabilityWith(kmType)

    return if (nullableByAnnotations)
        this
    else
        notNullable()
}