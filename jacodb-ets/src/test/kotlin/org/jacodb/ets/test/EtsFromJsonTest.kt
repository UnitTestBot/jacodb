...
class EtsFromJsonTest {
    ...
    fun testLoadEtsProject() {
        val res = "/projects/Demo_Calc/source/entry"
    ...
    @Test
    fun testMethodWithTryCatch() {
        val path = "/samples/source/lang/try-catch.ts"
        val res = getResourcePath(path)
        val file = loadEtsFileAutoConvert(res)
        ...
        assert(trap.tryBlocks.isNotEmpty()) { "Try blocks should not be empty" }
        assert(trap.catchBlocks.isNotEmpty()) { "Catch blocks should not be empty" }

        logger.info { "Try-catch trap: tryBlocks=${trap.tryBlocks.size}, catchBlocks=${trap.catchBlocks.size}" }
    }
}
