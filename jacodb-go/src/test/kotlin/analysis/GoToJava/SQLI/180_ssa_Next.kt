...
class ssa_Next : ssaToJacoExpr, ssaToJacoValue {

	var register: ssa_register? = null
	var Iter: Any? = null
	var IsString: Boolean? = null
	...
	override fun createJacoDBValue(parent: GoMethod): GoValue {
		if (structToPtrMap.containsKey(this) && ptrToJacoMap.containsKey(structToPtrMap[this])) {
            return ptrToJacoMap[structToPtrMap[this]] as GoNextExpr
        }
...
fun read_ssa_Next(buffReader: BufferedReader, id: Int): ssa_Next {
	val res = ssa_Next()
    if (id != -1) {
        if (ptrMap.containsKey(id)) {
            return ptrMap[id] as ssa_Next
        }
        ptrMap[id] = res
    ...
    res.Iter = mapDec[readType]?.invoke(buffReader, id) as Any?
    ...
    res.IsString = mapDec[readType]?.invoke(buffReader, id) as Boolean?
