...
class ssa_SliceToArrayPointer : ssaToJacoExpr, ssaToJacoValue {
	...
	override fun createJacoDBValue(parent: GoMethod): GoValue {
		if (structToPtrMap.containsKey(this) && ptrToJacoMap.containsKey(structToPtrMap[this])) {
            return ptrToJacoMap[structToPtrMap[this]] as GoSliceToArrayPointerExpr
        }
...
fun read_ssa_SliceToArrayPointer(buffReader: BufferedReader, id: Int): ssa_SliceToArrayPointer {
	val res = ssa_SliceToArrayPointer()
    if (id != -1) {
        if (ptrMap.containsKey(id)) {
            return ptrMap[id] as ssa_SliceToArrayPointer
        }
        ptrMap[id] = res
