...
class ssa_ChangeType : ssaToJacoExpr, ssaToJacoValue {
	...
	override fun createJacoDBValue(parent: GoMethod): GoValue {
		if (structToPtrMap.containsKey(this) && ptrToJacoMap.containsKey(structToPtrMap[this])) {
            return ptrToJacoMap[structToPtrMap[this]] as GoChangeTypeExpr
        }
...
fun read_ssa_ChangeType(buffReader: BufferedReader, id: Int): ssa_ChangeType {
	val res = ssa_ChangeType()
    if (id != -1) {
        if (ptrMap.containsKey(id)) {
            return ptrMap[id] as ssa_ChangeType
        }
        ptrMap[id] = res
