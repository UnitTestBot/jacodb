...
class ssa_MakeMap : ssaToJacoExpr, ssaToJacoValue {
	...
	override fun createJacoDBValue(parent: GoMethod): GoValue {
		if (structToPtrMap.containsKey(this) && ptrToJacoMap.containsKey(structToPtrMap[this])) {
            return ptrToJacoMap[structToPtrMap[this]] as GoMakeMapExpr
        }
...
fun read_ssa_MakeMap(buffReader: BufferedReader, id: Int): ssa_MakeMap {
	val res = ssa_MakeMap()
    if (id != -1) {
        if (ptrMap.containsKey(id)) {
            return ptrMap[id] as ssa_MakeMap
        }
        ptrMap[id] = res
    ...
    res.Reserve = mapDec[readType]?.invoke(buffReader, id) as Any?
