...
class ssa_IndexAddr : ssaToJacoExpr, ssaToJacoValue {
	...
	override fun createJacoDBValue(parent: GoMethod): GoValue {
		if (structToPtrMap.containsKey(this) && ptrToJacoMap.containsKey(structToPtrMap[this])) {
            return ptrToJacoMap[structToPtrMap[this]] as GoIndexAddrExpr
        }
...
fun read_ssa_IndexAddr(buffReader: BufferedReader, id: Int): ssa_IndexAddr {
	val res = ssa_IndexAddr()
    if (id != -1) {
        if (ptrMap.containsKey(id)) {
            return ptrMap[id] as ssa_IndexAddr
        }
        ptrMap[id] = res
