...
class ssa_Range : ssaToJacoExpr, ssaToJacoValue {
	...
	override fun createJacoDBValue(parent: GoMethod): GoValue {
		if (structToPtrMap.containsKey(this) && ptrToJacoMap.containsKey(structToPtrMap[this])) {
            return ptrToJacoMap[structToPtrMap[this]] as GoRangeExpr
        }
...
fun read_ssa_Range(buffReader: BufferedReader, id: Int): ssa_Range {
	val res = ssa_Range()
    if (id != -1) {
        if (ptrMap.containsKey(id)) {
            return ptrMap[id] as ssa_Range
        }
        ptrMap[id] = res
