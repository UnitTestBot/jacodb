...
class ssa_FieldAddr : ssaToJacoExpr, ssaToJacoValue {
	...
	override fun createJacoDBValue(parent: GoMethod): GoValue {
		if (structToPtrMap.containsKey(this) && ptrToJacoMap.containsKey(structToPtrMap[this])) {
            return ptrToJacoMap[structToPtrMap[this]] as GoFieldAddrExpr
        }
...
fun read_ssa_FieldAddr(buffReader: BufferedReader, id: Int): ssa_FieldAddr {
	val res = ssa_FieldAddr()
    if (id != -1) {
        if (ptrMap.containsKey(id)) {
            return ptrMap[id] as ssa_FieldAddr
        }
        ptrMap[id] = res
    ...
    res.Field = mapDec[readType]?.invoke(buffReader, id) as Long?
