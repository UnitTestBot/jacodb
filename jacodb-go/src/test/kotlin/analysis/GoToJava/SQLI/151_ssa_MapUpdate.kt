...
class ssa_MapUpdate : ssaToJacoInst {
...
fun read_ssa_MapUpdate(buffReader: BufferedReader, id: Int): ssa_MapUpdate {
	val res = ssa_MapUpdate()
    if (id != -1) {
        if (ptrMap.containsKey(id)) {
            return ptrMap[id] as ssa_MapUpdate
        }
        ptrMap[id] = res
    ...
    res.Map = mapDec[readType]?.invoke(buffReader, id) as Any?
