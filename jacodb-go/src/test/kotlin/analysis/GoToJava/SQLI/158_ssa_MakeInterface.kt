...
class ssa_MakeInterface : ssaToJacoExpr, ssaToJacoValue {
	...
	override fun createJacoDBValue(parent: GoMethod): GoValue {
		if (structToPtrMap.containsKey(this) && ptrToJacoMap.containsKey(structToPtrMap[this])) {
            return ptrToJacoMap[structToPtrMap[this]] as GoMakeInterfaceExpr
        }
...
fun read_ssa_MakeInterface(buffReader: BufferedReader, id: Int): ssa_MakeInterface {
	val res = ssa_MakeInterface()
    if (id != -1) {
        if (ptrMap.containsKey(id)) {
            return ptrMap[id] as ssa_MakeInterface
        }
        ptrMap[id] = res
