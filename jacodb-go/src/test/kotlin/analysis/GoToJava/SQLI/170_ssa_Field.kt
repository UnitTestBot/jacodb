...
class ssa_Field : ssaToJacoExpr, ssaToJacoValue {
	...
	override fun createJacoDBValue(parent: GoMethod): GoValue {
		if (structToPtrMap.containsKey(this) && ptrToJacoMap.containsKey(structToPtrMap[this])) {
            return ptrToJacoMap[structToPtrMap[this]] as GoFieldExpr
        }
...
fun read_ssa_Field(buffReader: BufferedReader, id: Int): ssa_Field {
	val res = ssa_Field()
    if (id != -1) {
        if (ptrMap.containsKey(id)) {
            return ptrMap[id] as ssa_Field
        }
        ptrMap[id] = res
