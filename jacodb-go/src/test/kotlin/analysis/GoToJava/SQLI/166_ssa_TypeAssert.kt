...
class ssa_TypeAssert : ssaToJacoExpr, ssaToJacoValue {

	var register: ssa_register? = null
	var X: Any? = null
	var AssertedType: Any? = null
	...
	override fun createJacoDBValue(parent: GoMethod): GoValue {
		if (structToPtrMap.containsKey(this) && ptrToJacoMap.containsKey(structToPtrMap[this])) {
            return ptrToJacoMap[structToPtrMap[this]] as GoTypeAssertExpr
        }
...
fun read_ssa_TypeAssert(buffReader: BufferedReader, id: Int): ssa_TypeAssert {
	val res = ssa_TypeAssert()
    if (id != -1) {
        if (ptrMap.containsKey(id)) {
            return ptrMap[id] as ssa_TypeAssert
        }
        ptrMap[id] = res
    ...
    res.AssertedType = mapDec[readType]?.invoke(buffReader, id) as Any?
