...
class ssa_Lookup : ssaToJacoExpr, ssaToJacoValue {
	...
	override fun createJacoDBValue(parent: GoMethod): GoValue {
		if (structToPtrMap.containsKey(this) && ptrToJacoMap.containsKey(structToPtrMap[this])) {
            return ptrToJacoMap[structToPtrMap[this]] as GoLookupExpr
        }
...
fun read_ssa_Lookup(buffReader: BufferedReader, id: Int): ssa_Lookup {
	val res = ssa_Lookup()
    if (id != -1) {
        if (ptrMap.containsKey(id)) {
            return ptrMap[id] as ssa_Lookup
        }
        ptrMap[id] = res
