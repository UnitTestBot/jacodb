...
class ssa_MakeChan : ssaToJacoExpr, ssaToJacoValue {
	...
	override fun createJacoDBValue(parent: GoMethod): GoValue {
		if (structToPtrMap.containsKey(this) && ptrToJacoMap.containsKey(structToPtrMap[this])) {
            return ptrToJacoMap[structToPtrMap[this]] as GoMakeChanExpr
        }
...
fun read_ssa_MakeChan(buffReader: BufferedReader, id: Int): ssa_MakeChan {
	val res = ssa_MakeChan()
    if (id != -1) {
        if (ptrMap.containsKey(id)) {
            return ptrMap[id] as ssa_MakeChan
        }
        ptrMap[id] = res
    ...
    res.Size = mapDec[readType]?.invoke(buffReader, id) as Any?
