...
class ssa_MakeSlice : ssaToJacoExpr, ssaToJacoValue {
	...
	override fun createJacoDBValue(parent: GoMethod): GoValue {
		if (structToPtrMap.containsKey(this) && ptrToJacoMap.containsKey(structToPtrMap[this])) {
            return ptrToJacoMap[structToPtrMap[this]] as GoMakeSliceExpr
        }
...
fun read_ssa_MakeSlice(buffReader: BufferedReader, id: Int): ssa_MakeSlice {
	val res = ssa_MakeSlice()
    if (id != -1) {
        if (ptrMap.containsKey(id)) {
            return ptrMap[id] as ssa_MakeSlice
        }
        ptrMap[id] = res
    ...
    res.Len = mapDec[readType]?.invoke(buffReader, id) as Any?
    ...
    res.Cap = mapDec[readType]?.invoke(buffReader, id) as Any?
