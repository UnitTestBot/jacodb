...
class ssa_Extract : ssaToJacoExpr, ssaToJacoValue {
	...
	override fun createJacoDBValue(parent: GoMethod): GoValue {
		if (structToPtrMap.containsKey(this) && ptrToJacoMap.containsKey(structToPtrMap[this])) {
            return ptrToJacoMap[structToPtrMap[this]] as GoExtractExpr
        }
...
fun read_ssa_Extract(buffReader: BufferedReader, id: Int): ssa_Extract {
	val res = ssa_Extract()
    if (id != -1) {
        if (ptrMap.containsKey(id)) {
            return ptrMap[id] as ssa_Extract
        }
        ptrMap[id] = res
    ...
    res.Tuple = mapDec[readType]?.invoke(buffReader, id) as Any?
