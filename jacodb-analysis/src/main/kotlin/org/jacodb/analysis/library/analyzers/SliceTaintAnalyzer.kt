 ...
 * This analysis computes the program slice (set of program instructions) that may affect the values
 * at sink instructions.
 *
 * @property sinks is a set of [JcInst] that have been identified as points of interest, e.g. as a result
 * of another analysis.
 */
class SliceTaintAnalyzer(graph: JcApplicationGraph, val sinks: Set<JcInst>, maxPathLength: Int) : AbstractAnalyzer(graph) {
    override val flowFunctions: FlowFunctionsSpace = SliceTaintBackwardFunctions(graph, sinks, maxPathLength)
    ...
    override fun handleNewEdge(edge: IfdsEdge): List<AnalysisDependentEvent> = buildList {
        ...
        val variableIsAssignedToSmth = fact0 is SliceDataFlowNode
                && inst is JcAssignInst
                && inst.lhv.toPathOrNull()?.startsWith(fact0.variable) == true
...
fun SliceTaintAnalyzerFactory(sinks: Set<JcInst>, maxPathLength: Int) = AnalyzerFactory { graph ->
    SliceTaintAnalyzer(graph, sinks, maxPathLength)
}

private class SliceTaintBackwardFunctions(
    graph: JcApplicationGraph,
    val sinks: Set<JcInst>,
    maxPathLength: Int,
