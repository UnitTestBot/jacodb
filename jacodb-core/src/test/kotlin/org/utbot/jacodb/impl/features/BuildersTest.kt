...
import org.utbot.jacodb.impl.builders.Hierarchy.HierarchyInterface
import org.utbot.jacodb.impl.builders.Interfaces.Interface
...
class BuildersTest : BaseTest() {
    ...
    fun `simple find builders`() {
        val builders = ext.findBuildMethods(cp.findClass<org.utbot.jacodb.impl.builders.Simple>()).toList()
    ...
    fun `works for methods returns subclasses`() {
        ...
        assertEquals(1, builders.size)
        assertEquals("org.utbot.jacodb.impl.builders.Hierarchy#build", builders.first().loggable)
    }
