...
class InnerTypesTest : BaseTypesTest() {
    ...
    fun `inner classes linked to method`() {
        val classWithInners = findType<org.utbot.jacodb.impl.types.InnerClasses<*>>()
    ...
    fun `get not parameterized inner types`() {
        val innerClasses = findType<org.utbot.jacodb.impl.types.InnerClasses<*>>().innerTypes
    ...
    private fun field(fieldName: String): JcClassType {
        return findType<org.utbot.jacodb.impl.types.InnerClasses<*>>().fields.first {
            it.name == fieldName
